...
import { CacheData, HistoricoItem } from '@/types';
...
export default function RelatorioMovimentacoes({ caches }: RelatorioProps) {
  ...
  let historicoFiltrado: HistoricoItem[] = [...caches.historico];
  ...
  if (tipoFiltro) historicoFiltrado = historicoFiltrado.filter(h => h.tipo === tipoFiltro);
  if (usuarioFiltro) historicoFiltrado = historicoFiltrado.filter(h => h.usuario === usuarioFiltro);
  
  // CORREÇÃO DE ARITMÉTICA DE DATA AQUI:
  const historicoOrdenado = historicoFiltrado.sort((a, b) => (b.data?.toDate().getTime() || 0) - (a.data?.toDate().getTime() || 0));
  ...
  const handleExport = () => {
    const dataToExport = historicoOrdenado.map(h => {
      ...
      return {
        'Data': h.data ? h.data.toDate().toLocaleString('pt-BR') : 'N/A', 'Produto': produto?.nome || 'N/A', 'Tipo': h.tipo,
        'Quantidade': h.quantidade, 'Serial Numbers': h.serialNumbers?.join(', ') || 'N/A', 'Origem': origem, 'Destino': destino, 'Usuario': usuario,
      };
    });
    ...
    <div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-4">
        <div>
          ...
          <input type="date" value={dataInicio} onChange={e => setDataInicio(e.target.value)} className="p-2 w-full border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-700 text-gray-900 dark:text-white focus:ring-blue-500 focus:border-blue-500" />
        </div>
        <div>
          <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Data Fim</label>
          <input type="date" value={dataFim} onChange={e => setDataFim(e.target.value)} className="p-2 w-full border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-700 text-gray-900 dark:text-white focus:ring-blue-500 focus:border-blue-500" />
        </div>
        <div>
          <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Tipo</label>
          <select value={tipoFiltro} onChange={e => setTipoFiltro(e.target.value)} className="p-2 w-full border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-700 text-gray-900 dark:text-white focus:ring-blue-500 focus:border-blue-500">
        ...
        <div>
          ...
          <select value={usuarioFiltro} onChange={e => setUsuarioFiltro(e.target.value)} className="p-2 w-full border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-700 text-gray-900 dark:text-white focus:ring-blue-500 focus:border-blue-500">
            <option value="">Todos</option>
            {Array.from(caches.usuarios.values()).sort((a, b) => a.username.localeCompare(b.username)).map(u => <option key={u.id} value={u.id}>{u.username}</option>)}
          </select>
        </div>
      </div>
