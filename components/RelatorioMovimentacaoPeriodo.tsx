...
export default function RelatorioMovimentacoesPeriodo({ caches }: RelatorioProps) {
  const [currentPage, setCurrentPage] = useState(1);

  const historicoOrdenado = [...caches.historico].sort((a, b) =>
    (b.data?.toDate().getTime() || 0) - (a.data?.toDate().getTime() || 0)
  );
    ...
    <div>
      <table className="min-w-full bg-white dark:bg-gray-800 text-sm text-left text-gray-700 dark:text-gray-300">
        ...
        <tbody className="divide-y dark:divide-gray-700">
          {historicoPaginado.map((h: HistoricoItem) => {
