...
export default function ModalMovimentar({ isOpen, onClose, produto, caches }: ModalMovimentarProps) {
  ...
  const sortedLocalidades = useMemo(() => 
    Array.from(caches.localidades.values()).sort((a, b) => a.nome.localeCompare(b.nome)),
  [caches.localidades]);
    ...
    try {
      if (produto.tipoControle === 'Quantidade' || !produto.tipoControle) {
        await runTransaction(db, async (transaction) => {
          ...
          let histData: any = {
            produtoId: produto.id, tipo, quantidade: qtd, data: serverTimestamp(),
            usuario: auth.currentUser?.uid
          };
          if (tipo === 'ENTRADA') {
            const destinoId = localPadrao?.id;
            if (!destinoId) throw new Error("Local padrão 'ITC BRASIL' não encontrado.");
            histData.localidadeDestinoId = destinoId;
            const estoqueId = `${produto.id}_${destinoId}`;
            const estoqueRef = doc(db, "estoque", estoqueId);
            const estoqueDoc = await transaction.get(estoqueRef);
            if (!estoqueDoc.exists()) {
              transaction.set(estoqueRef, { produtoId: produto.id, localidadeId: destinoId, quantidade: qtd });
            } else {
            ...
            if (!estoqueOrigemDoc.exists()) throw new Error("Estoque na origem não existe.");
            ...
            if (tipo === 'TRANSFERENCIA') {
              ...
              const estoqueDestinoId = `${produto.id}_${localidadeDestinoId}`;
              ...
              if (!estoqueDestinoDoc.exists()) {
                transaction.set(estoqueDestinoRef, { produtoId: produto.id, localidadeId: localidadeDestinoId, quantidade: qtd });
              } else {
        ...
        const histRef = doc(collection(db, "historico"));
        let histData: any = {
          produtoId: produto.id, tipo, data: serverTimestamp(),
          usuario: auth.currentUser?.uid
        };
        if (tipo === 'ENTRADA') {
          const destinoId = localPadrao?.id;
          ...
          histData = { ...histData, localidadeDestinoId: destinoId, quantidade: snsValidos.length, serialNumbers: snsValidos };
          snsValidos.forEach(sn => {
            const newUnidadeRef = doc(collection(db, "unidadesEstoque"));
            batch.set(newUnidadeRef, {
              produtoId: produto.id, serialNumber: sn.trim(), localidadeId: destinoId,
              status: 'Em Estoque', createdAt: serverTimestamp(),
            });
          });
        } else {
          if (!localidadeOrigemId) throw new Error("Local de origem é obrigatório.");
      ...
      const detalhesLog: any = {
        ...
        quantidade: produto.tipoControle === 'Serial Number' ? unidadesSelecionadas.length : quantidade,
      };
      if (tipo === 'ENTRADA') {
        detalhesLog.origem = 'EXTERNO';
        detalhesLog.destino = localPadrao?.nome || 'N/A';
        if (produto.tipoControle === 'Serial Number') {
          detalhesLog.serialNumbers = serialNumbers.filter(sn => sn && sn.trim().length > 0);
        }
      } else if (tipo === 'SAIDA') {
        detalhesLog.origem = caches.localidades.get(localidadeOrigemId)?.nome || 'N/A';
        ...
        detalhesLog.origem = caches.localidades.get(localidadeOrigemId)?.nome || 'N/A';
        detalhesLog.destino = caches.localidades.get(localidadeDestinoId)?.nome || 'N/A';
        if (produto.tipoControle === 'Serial Number') {
          detalhesLog.serialNumbers = unidadesSelecionadas;
        }
      }
      ...
      {tipo === 'TRANSFERENCIA' && (
        <div>
          ...
          <select value={localidadeDestinoId} onChange={(e) => setLocalidadeDestinoId(e.target.value)} required className="mt-1 block w-full p-2 border border-gray-400 rounded-lg dark:bg-gray-700 dark:border-gray-600 dark:text-white">
            <option value="">Selecione...</option>
            {sortedLocalidades.map((l) => <option key={l.id} value={l.id!}>{l.nome}</option>)}
          </select>
        </div>
      )}
    </>
        ...
        <>
          ...
          {localidadeOrigemId && (
            <div className="max-h-48 overflow-y-auto border dark:border-gray-600 rounded-lg p-2 space-y-1 mt-2">
                ...
                {(caches.unidadesEstoque || [])
                  .filter(u => u.localidadeId === localidadeOrigemId && u.produtoId === produto?.id)
                  .map(u => (
                    <label key={u.id} className="flex items-center space-x-2 p-2 rounded-md hover:bg-gray-100 dark:hover:bg-gray-700 cursor-pointer">
                        ...
                        <span className="font-mono text-sm text-gray-800 dark:text-gray-200">{u.serialNumber}</span>
                    </label>
                ))}
            </div>
          )}
