...
export default function DashboardPage() {
  ...
  useEffect(() => {
    ...
    const collectionsToListen: (keyof CacheData)[] = ['produtos', 'estoque', 'unidadesEstoque', 'historico', 'usuarios', 'localidades', 'fabricantes', 'categorias', 'fornecedores', 'projetos'];
  ...
  const { totalProdutos, itensEstoqueBaixo, itensEstoqueProximo } = useMemo(() => {
    ...
    produtosArray.forEach(p => {
      ...
      if (estoqueMinimo > 0) {
        ...
        if (totalEstoque <= estoqueMinimo) {
          baixo.push(p);
        } else if (totalEstoque > estoqueMinimo && totalEstoque <= estoqueMinimo * 1.1) {
          proximo.push(p);
        }
      }
    });

    return { totalProdutos: produtosArray.length, itensEstoqueBaixo: baixo, itensEstoqueProximo: proximo };
  }, [caches.produtos, caches.estoque, caches.unidadesEstoque]);
  ...
  return (
    <div>
      ...
      <Modal isOpen={modalState.estoqueBaixo} onClose={() => setModalState(s => ({ ...s, estoqueBaixo: false }))} title="Itens com Estoque Baixo">
        <TabelaEstoqueBaixo produtos={itensEstoqueBaixo} estoque={caches.estoque} unidadesEstoque={caches.unidadesEstoque} onDetailsClick={handleOpenDetalhes} />
      </Modal>

      <Modal isOpen={modalState.estoqueProximo} onClose={() => setModalState(s => ({ ...s, estoqueProximo: false }))} title="Itens Próximos do Estoque Mínimo">
        <TabelaEstoqueProximo produtos={itensEstoqueProximo} estoque={caches.estoque} unidadesEstoque={caches.unidadesEstoque} onDetailsClick={handleOpenDetalhes} />
      </Modal>
